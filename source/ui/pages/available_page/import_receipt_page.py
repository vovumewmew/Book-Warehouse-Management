...
class ImportReceiptPage(BasePage):
    def __init__(self, page: ft.Page, change_page_func, **kwargs):
        super().__init__("Phiếu Nhập Sách")
        ...
        phieunhap_dao = PhieuNhapSachDAO(db)
        chitiet_dao = ChiTietPhieuNhapDAO(db)
        sach_dao = SachDAO(db)
        ...
        self.phieunhap_service.set_chitiet_service(self.chitiet_service) # Liên kết 2 service
        self.phieunhap_service.set_sach_service(self.sach_service) # Liên kết service sách

        # --- Nút thêm phiếu nhập ---
        add_button = AddNewImportReceipt(
            page=self.page,
        ...
        # --- Thanh tìm kiếm ---
        search_bar = SearchBarImportReceipt(
            page=self.page,
            service=self.phieunhap_service,
        ...
        self.receipt_table = ft.DataTable(
            columns=[
                ...
                ft.DataColumn(ft.Text("Nguồn nhập", weight="bold")),
                ft.DataColumn(ft.Text("Ngày Nhập", weight="bold")),
    ...
    def load_receipts(self, receipts=None):
        """Tải dữ liệu phiếu nhập và cập nhật DataTable."""
        if receipts is None:
            receipts = self.phieunhap_service.get_all()
        ...
        for receipt in receipts:
            ...
            total_quantity = sum(detail.SoLuong for detail in receipt.Danhsachchitietnhap)
            total_price = sum(detail.ThanhTien for detail in receipt.Danhsachchitietnhap)

            # Tạo một hàm nội bộ để bắt giá trị 'receipt' cho mỗi vòng lặp
            new_table.rows.append(
                ft.DataRow(
                    cells=[
                        ft.DataCell(ft.Text(receipt.ID_PhieuNhap)),
                        ft.DataCell(ft.Text(receipt.TenNhanVien)),
                        ft.DataCell(ft.Text(receipt.TenNguonNhap)),
                        ft.DataCell(ft.Text(receipt.NgayNhap.strftime('%d/%m/%Y'))),
