...
class DeleteImportReceiptButton(ft.ElevatedButton):
    ...
    def confirm_delete(self, e):
        ...
        self._show_dialog(
            title="Xác nhận xóa",
            message=f"Bạn có chắc chắn muốn xóa phiếu '{self._receipt.ID_PhieuNhap}'?\nHành động này sẽ hoàn tác số lượng sách trong kho.",
            is_confirm=True
        )
    ...
    def delete_action(self, e):
        ...
        self.close_dialog()
        try:
            success = self.service.delete(self._receipt.ID_PhieuNhap)
            if success:
                # Thay vì gọi _show_dialog, gọi hàm on_deleted trực tiếp
                # để đóng form chi tiết và reload bảng.
                # Thông báo thành công sẽ hiển thị trên trang chính.
                if self.on_deleted: 
                    self.on_deleted(f"Đã xóa thành công phiếu '{self._receipt.ID_PhieuNhap}'.")
            else:
                # Trường hợp service trả về False mà không ném Exception
                self._show_dialog("Lỗi", "Xóa phiếu thất bại do một lỗi không xác định.", is_error=True)
        except Exception as ex:
            # Bắt lỗi từ service (ví dụ: không đủ hàng tồn kho)
    ...
    def _show_dialog(self, title: str, message: str, is_confirm: bool = False, is_error: bool = False):
        ...
        # Chọn màu nền dựa trên loại dialog
        if is_error:
            bgcolor = "#F8BBD0"
            title_color = "#C2185B"
        elif is_confirm:
