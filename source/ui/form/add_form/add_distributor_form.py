...
class AddDistributorForm(AddFormBase):
    def __init__(self, on_submit=None, on_close=None, on_success=None):
        super().__init__(title="Thêm nhà phân phối", on_submit=on_submit, on_close=on_close)
        self.on_success = on_success

        # --- Các trường nhập liệu ---
        self.id_npp_field = self.add_field("Mã nhà phân phối")
        self.ten_co_so_field = self.add_field("Tên nhà phân phối")
        ...
        npp_dao = NhaPhanPhoiDAO(self.db)
        self.npp_service = NhaPhanPhoiService(npp_dao)
    ...
    def _handle_submit(self, e):
        try:
            data = {
                "ID_NguonXuat": self.id_npp_field.value.strip(),
            ...
            npp = NhaPhanPhoi(
                ...
                HinhAnh=self.image_file or get_absolute_path("source/ui/picture/contributors_pic/contributor_default_pic.jpg")
            )
            ...
            if "Mã nhà phân phối" in msg: field = self.id_npp_field
            elif "Tên cơ sở" in msg: field = self.ten_co_so_field
