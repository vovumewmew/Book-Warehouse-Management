...
class SupplierForm(DisplayFormBase):
    def __init__(self, supplier, page=None, on_close=None, mode="available", **kwargs):
        """
        supplier: đối tượng NguonNhapSach
        page: ft.Page hiện tại
        """
        self.page = page
        ...
        self.supplier_dao = NguonNhapSachDAO(db_conn)
        self.supplier_service = NguonNhapSachService(self.supplier_dao)

        # --- Mapping label → attribute ---
        self.label_to_attr = {
            "Mã nhà cung cấp": "ID_NguonNhap",
            "Tên nhà cung cấp": "TenCoSo",
            "Hình thức nhập": "HinhThucNhap",
            "Địa chỉ": "DiaChi",
            "Số điện thoại": "SoDienThoai",
            "Email": "Email",
            "Trạng thái NCC": "TrangThaiNCC",
            "Tính khả dụng": "TinhKhaDung",
        }

        # --- Tạo fields hiển thị từ object supplier ---
        fields = {label: getattr(supplier, attr, "") for label, attr in self.label_to_attr.items()}

        # --- Gọi super().__init__() sau khi đã có service ---
        super().__init__(
            title=f"{supplier.TenCoSo}",
            fields=fields,
            width=450,
            height=550,
            on_close=on_close
    ...
    def build_form(self):
        ...
        # --- Lấy nút Đóng cũ và đặt vào Row với nút Sửa ---
    ...
    def restore_action(self, e):
        ...
        self._show_confirmation(
            ...
            message=f"Bạn có chắc chắn muốn phục hồi nhà cung cấp '{self._supplier.TenCoSo}' không?",
            on_confirm=do_restore
        )

    def delete_permanently_action(self, e):
        def do_delete():
            self.supplier_service.completely_delete(self._supplier.ID_NguonNhap)
            self.show_snackbar(f"Đã xóa vĩnh viễn nhà cung cấp '{self._supplier.TenCoSo}'.")
            super(SupplierForm, self)._handle_close(e)

        self._show_confirmation(
            title="Xác nhận xóa vĩnh viễn",
            message=f"Hành động này không thể hoàn tác. Bạn có chắc muốn xóa vĩnh viễn nhà cung cấp '{self._supplier.TenCoSo}' không?",
